...
static slot_reuse slot_probe_reuse_locked(server *s, server_slot *slot,
    ...
    /* The gate is ds4_engine_can_rewind(), not a hard-coded family test: it
     * also admits Metal DSpark, whose snapshots ds4_session_rewind() can
     * restore.  Keeping the predicate in one place is what upstream's shared
 ...
 *
 * A partial prefix is rewound as well: the live state is truncated to the
 * common prefix and the remaining prompt tokens are re-evaluated, which still
 * beats re-prefilling the whole conversation.  Upstream narrowed this to a
 * strict truncation (common != prompt_len -> -1); the partial case is restored
 * because it is the one a client replaying a conversation with a changed tail
 * actually hits.
 *
 * The caller re-validates the token/image prefix before materializing. */
static int live_prefix_rewind_target(bool backend_can_rewind,
                                     int old_pos, int prompt_len, int common,
                                     bool *full_prefix) {
...
static const char *const *server_model_ids_for_family(bool ds41, bool glm53, bool glm_dsa,
                                                      bool qwen4, size_t *count) {
    static const char *const ds41_ids[] = {
        "deepseek-v4.1-flash"
    };
    /* 合并上游 Qwen3.8 支持 */
    static const char *const qwen4_ids[] = {
        "qwen3.8-flash-next", "qwen3.8-flash-next-chat", "qwen3.8-flash-next-reasoner"
    };
    ...
    if (qwen4) {
        *count = sizeof(qwen4_ids) / sizeof(qwen4_ids[0]);
        return qwen4_ids;
    }
    if (ds41) {
        *count = sizeof(ds41_ids) / sizeof(ds41_ids[0]);
        return ds41_ids;
    }
    if (glm53) {
...
static bool send_models(server *s, int fd) {
    ...
    if (ds4_engine_is_deepseek41(s->engine)) {
        ...
        for (size_t qi = 0; qi < sizeof(qwen_ids) / sizeof(qwen_ids[0]); qi++) {
            if (!first) buf_putc(&b, ',');
            append_model_json(&b, s, qwen_ids[qi]);
            first = false;
        }
...
static void test_live_prefix_rewind_target(void) {
    bool full = false;
    /* Whole prompt resident: rewind one short of it and let the sampler take
    ...
    /* Partial prefix: truncate to the common prefix, re-evaluate the suffix. */
...
static void test_model_catalog_matches_loaded_variant(void) {
    ...
    const char *const *glm53 = server_model_ids_for_family(false, true, true, false, &count);
    TEST_ASSERT(count == 3);
    ...
    const char *const *glm52 = server_model_ids_for_family(false, false, true, false, &count);
    TEST_ASSERT(count == 3);
