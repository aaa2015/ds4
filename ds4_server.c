...
static void test_model_catalog_matches_loaded_variant(void) {
    ...
    const char *const *ds41 = server_model_ids_for_family(true, false, false, &count);
    TEST_ASSERT(count == 1);
    TEST_ASSERT(!strcmp(ds41[0], "deepseek-v4.1-flash"));

    const char *const *glm53 = server_model_ids_for_family(false, true, true, &count);
    TEST_ASSERT(count == 3);
    ...
    const char *const *glm52 = server_model_ids_for_family(false, false, true, &count);
    TEST_ASSERT(count == 3);
    TEST_ASSERT(!strcmp(glm52[0], "glm-5.2"));
    TEST_ASSERT(!strcmp(glm52[1], "glm-5.2-chat"));
    TEST_ASSERT(!strcmp(glm52[2], "glm-5.2-reasoner"));

    const char *const *deepseek = server_model_ids_for_family(false, false, false, &count);
    TEST_ASSERT(count == 2);
